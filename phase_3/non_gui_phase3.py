...
def Run_program():
    i_cache_size=int(input("Enter size of instruction cache (in bytes) "))
    i_block_size=int(input("Enter size of block for instruction cache (in bytes) "))
    i_ways=int(input("Enter set associativity "))
    print("-----------------------------------------------------")
    d_cache_size = int(input("Enter size of data cache (in bytes) "))
    d_block_size = int(input("Enter size of block for data cache (in bytes) "))
    d_ways = int(input("Enter set associativity "))
    print("-----------------------------------------------------")
    print("Select 1 for Non-pipelined")
    print("Select 2 for Pipelined without data forwarding")
    print("Select 3 for Pipelined with data forwarding")

    choice = int(input("Please enter your choice : "))

    if choice == 1:
        print("/---------Non-Pipelined Running-------------/\n")
        Execute = execute(i_cache_size,i_block_size,i_ways,d_cache_size,d_block_size,d_ways)
        ...
        if (MC != ""):
            ...
            print("No of cache access :", Execute.Instruction_cache.number_of_hit + Execute.Instruction_cache.number_of_miss + Execute.Data_cache.number_of_hit+Execute.Data_cache.number_of_miss)
            print("No of Cache hit :", Execute.Instruction_cache.number_of_hit + Execute.Data_cache.number_of_hit )
            print("No of Cache miss :", Execute.Instruction_cache.number_of_miss + Execute.Data_cache.number_of_miss)
        else:
        ...
        Execute = pipelineExecute(i_cache_size,i_block_size,i_ways,d_cache_size,d_block_size,d_ways)
        if choice == 2:
...
class execute:
    ...
    def __init__(self,i_cache_size,i_block_size,i_ways,d_cache_size,d_block_size,d_ways):

        self.sp = 0x7ffffff0
        """
            for creating the register file
        """
        ...
        self.instruction = 0
        self.Instruction_cache = cache(i_cache_size,i_block_size,i_ways)
        self.Data_cache=cache(d_cache_size,d_block_size,d_ways)
    ...
    def Code_run(self):
        ...
        print(self.Instruction_cache.returns())
        print("Data cache: ")
        print(self.Data_cache.returns())
        print("\n|-----------Execution of the program start-------|\n")
        while self.Instruction_cache.read_word(self.Program_counter,self.Memory) !=0 :
    ...
    def fetch(self):
        ...
        self.instruction = self.Instruction_cache.read_word(self.Program_counter,self.Memory)
    ...
    def checkFormat(self):
        ...
        # storing the part of the OP_code of every instruction
    ...
    def DecodeI(self):
        ...
        # extracting all the fields of the instruction
        ...
        print("function_3:" + self.funtion_3)
        ...
        if self.OP_code == "0000011":
            if (
                    self.funtion_3 == "000" or self.funtion_3 == "001" or self.funtion_3 == "101" or self.funtion_3 == "100"):
                self.memory_enable = 1
                no_match_found = 0
                self.muxY = 1
                self.ALU(ADD)  # all load ins lb, lh, lw
    ...
    def DecodeSB(self):
        ...
        """
            assigning the value of the control signal
        """
        self.write_enable = 0
        self.muxB = 0
        no_match_found = 1
    ...
    def memoryAccess(self):
        ...
        if self.memory_enable:
            if self.muxY == 1 and self.funtion_3 == "010":
                self.data = self.Data_cache.read_word(self.RZ,self.Memory)
                #self.data = self.Memory.readWord(self.RZ)  # lw

            if self.muxY != 1 and self.funtion_3 == "010":
                self.Data_cache.write_Word(self.RZ, self.RM, self.Memory)
                #self.Memory.writeWord(self.RZ, self.RM)

            if self.muxY == 1 and self.funtion_3 == "000":
                self.data = self.Data_cache.read_Byte(self.RZ,self.Memory)
                #self.data = self.Memory.readByte(self.RZ)  # lb

            if self.muxY != 1 and self.funtion_3 == "000":
                self.Data_cache.write_Byte(self.RZ, self.RM, self.Memory)
                #self.Memory.writeByte(self.RZ, self.RM)

            if self.muxY == 1 and self.funtion_3 == "001":
                self.data = self.Data_cache.read_DoubleByte(self.RZ,self.Memory)
                #self.data = self.Memory.readDoubleByte(self.RZ)  # lh

            if self.muxY != 1 and self.funtion_3 == "001":
                self.Data_cache.write_DoubleByte(self.RZ, self.RM, self.Memory)
...
class pipelineExecute:
    ...
    def __init__(self,i_cache_size,i_block_size,i_ways,d_cache_size,d_block_size,d_ways):
        ...
        self.imm = 0  # for the immediate field

        self.do_dataForwarding = 1  # for checking if data forwarding is allowed or not

        self.RDQueue = []  # Queue to store dest_reg_1(s) from all the instruction from proper pipelining
        self.branchTaken = 0
        ...
        self.Register = register()  # for the register file
        self.Memory = memory()  # for the memory
        self.RM = self.RZ = self.RB = self.RA = 0  # for the input1 register
    ...
    def Code_run(self):
        ...
        print("Number of the Cycle taken =", self.cycle)
    ...
    def runPipeLine(self):
        ...
        """
        if self.Instruction_cache.read_word(self.Program_counter,self.Memory) != 0:
        ...
        if (self.Instruction_cache.read_word(self.Program_counter,self.Memory) == 0) and (not self.fetch_run) and (not self.alu_run) and (not self.MEM_access_prog) and (not self.Decode_run):
    ...
    def DecodeI(self):
        ...
        if self.OP_code == "0000011":
            if (
                    self.funtion_3 == "000" or self.funtion_3 == "001" or self.funtion_3 == "010" or self.funtion_3 == "100" or self.funtion_3 == "101"):
...
class register:
    # initializing the register part
    def __init__(self):
        self.registers = {}
        for i in range(32):
            x = '{0:05b}'.format(i)
    ...
    # for updating the value of the registers
    def update(self, address, value):
        if not address == "00000":
            self.registers[address] = value

    # for returning the registers file
    def returns(self):
        return self.registers

    # for reading the value of the register
    def read(self, address):
        return self.registers[address]

    # for erasing the value stored in the registers
    def flush(self):
        for i in range(32):
            x = '{0:05b}'.format(i)
...
class cache:
    def __init__(self, cache_size, block_size, associativity):
        ...
        self.block_offset = math.log(block_size, 2)
        self.number_of_sets = math.ceil(self.cache_size / (self.block_size * self.associativity))
        self.cache = []
        self.number_of_words = self.block_size // 4
        ...
        for i in range(self.number_of_sets):
            ...
            0-tag represented by address of: address - address(%4) word or word0
            1-LRU
            2-data of address -address(%4) word---- word0
            3-data of address -address(%4) word---- word1
            4-data of address -address(%4) word---- word2
            5-data of address -address(%4) word---- word3
            .
            .
            .
            '''
            Block = [-1, -99999, {}]  # [Block_address, LRU_Value,  Block_Memory]
            Set = []
    ...
    def read_word(self, address, main_memory):
        ...
        flag = False  # for checking if we had found the data or not
        ...
        temp = str(BitArray(int=Block_address, length=32).bin)
        temp = temp[:-1* int(math.log(self.block_size, 2))]
        temp = temp[-1*int(math.log(self.number_of_sets, 2)):]
        
        set_index = int(temp, 2)%self.number_of_sets

        word = ''
        for i in range(self.associativity):
            if flag == False and self.cache[set_index][i][0] == Block_address:
                memory = self.cache[set_index][i][2]  # Block_memory

                word = ''
                for j in range(4):
                    if address + j in memory:
                        word = memory[address + j] + word
                    else:
                        word = "00000000" + word
                flag = True
                self.cache[set_index][i][1] = self.associativity

            else:
                self.cache[set_index][i][1] -= 1

        """

        if the word is found or not 

        """

        if (flag == True):
            print("set accessed : ",self.cache[set_index])
            self.number_of_hit +=  1 
            return BitArray(bin=word).int
        else:
            self.number_of_miss += 1 
            new_Block = [Block_address, self.associativity]
            Block_Memory = {}

            # taking data to the Block Memory from main Memory

            for i in range(self.block_size):
                Block_Memory[Block_address + i] = BitArray(int=main_memory.readByte(Block_address + i), length=8).bin

            new_Block.append(Block_Memory)

            # Printing the new created Block
            #print(new_Block)

            LRU_mini = 9999999999
            Block_mini_index = -1
            for i in range(self.associativity):
                if self.cache[set_index][i][1] < LRU_mini:
                    Block_mini_index = i
                    LRU_mini = self.cache[set_index][i][1]

            if(self.cache[set_index][Block_mini_index][0]<0):
                print("Victim Block is empty.")
            else:
                print("Victim Block: ", self.cache[set_index][Block_mini_index])
            self.cache[set_index][Block_mini_index] = new_Block

            word = ""
            for i in range(4):
                if address + i in Block_Memory:
                    word = Block_Memory[address + i] + word
                else:
                    word = "00000000" + word
            return BitArray(bin=word).int

    def read_Byte(self, address, main_memory):
        ...
        if (flag == True):
            ...
            word = Block_Memory[address]

            return BitArray(bin=word).int

    def read_DoubleByte(self, address, main_memory):
        '''
            Pass 32 bit address and extract index
        '''
    ...
    def write_Word(self, address, value, main_memory):
        ...
        for i in range(self.associativity):
            if flag == False and self.cache[set_index][i][0] == Block_address:
                ...
                word = BitArray(int=value, length=32).bin
                self.cache[set_index][i][2][address] = word[24:32]
                self.cache[set_index][i][2][address + 1] = word[16:24]
                self.cache[set_index][i][2][address + 2] = word[8:16]
                self.cache[set_index][i][2][address + 3] = word[0:8]
        ...
        if flag == True:
            print("set accessed : ",self.cache[set_index])
            self.number_of_hit += 1 
            main_memory.writeWord(address, value)
            return
        else:
            self.number_of_miss += 1
            main_memory.writeWord(address, value)
    ...
    def write_DoubleByte(self, address, value, main_memory):
        flag = False  # for checking if we had found the data or not
        ...
        for i in range(self.associativity):
            if flag == False and self.cache[set_index][i][0] == Block_address:
                ...
                word = BitArray(int=value, length=16).bin
                self.cache[set_index][i][2][address + 1] = word[8:16]
                self.cache[set_index][i][2][address] = word[0:8]
        ...
        if flag == True:
            print("Set accessed : ",self.cache[set_index])
            self.number_of_hit += 1
            main_memory.writeDoubleByte(address, value)
            return
        else:
            self.number_of_miss += 1 
            main_memory.writeDoubleByte(address, value)
    ...
    def write_Byte(self, address, value, main_memory):
        flag = False  # for checking if we had found the data or not
        ...
        for i in range(self.associativity):
            if flag == False and self.cache[set_index][i][0] == Block_address:
                ...
                word = BitArray(int=value, length=8).bin
                self.cache[set_index][i][2][address] = word
        ...
        if flag == True:
            print("set accessed : ",self.cache[set_index])
            self.number_of_hit += 1 
            main_memory.writeByte(address, value)
            return
        else:
            self.number_of_miss += 1 
            main_memory.writeByte(address, value)
